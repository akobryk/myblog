...
def log_post_updated_created_event(sender, **kwargs):
	...
	if kwargs['created']:
		logger.info('Post created: {} {} (ID: {}) by {}'.format(post.title, 
			post.updated, post.id, post.user))

	else:
		logger.info('Post updated: {} {} (ID: {}) by {}'.format(post.title, 
			post.updated, post.id, post.user))
...
def log_post_delete_event(sender, **kwargs):
	...
	post = kwargs['instance']
	logger.info('Post deleted: {} {} (ID: {}) by {}'.format(post.title, 
			post.updated, post.id, post.user))
...
def log_user_create_update_event(sender, **kwargs):
	...
	if kwargs['created']:
		logger.info('User created: {} {}(ID: {})'.format(user.username,
			user.email, user.id))
	else:
		logger.info('User updated: {} {}(ID: {})'.format(user.username, 
			user.email, user.id))
...
def log_user_delete_event(sender, **kwargs):
	...
	user = kwargs['instance']
	logger.info('User deleted: {} {}(ID: {})'.format(user.username, 
		user.email, user.id))
